...
class Causal_BVAE(nn.Module):
    def __init__(self):
        ...
        self.beta = 0  # nn.Parameter(torch.zeros(0))
    ...
    def IM(self, v, v_):
        ...
        imap = torch.abs(y.norm(dim=-1).detach() - y_.norm(dim=-1).detach())
    ...
    def forward(self, x, x_=None, imap=None):
        ...
        if self.training:
            if x_ is None:
                ...
                imap = self.IM(v, v_)
                transformed_map = self.alpha*imap+self.beta
        else:
            if imap is None:
                print("Enter influence map (imap)!")
            else:
                imap = imap.view(1, self.endo_vars)
                transformed_map = self.alpha*imap+self.beta
        ...
        return imap.sum(dim=0), imap_var, y, noisy_y, recon_x, mu, logvar
