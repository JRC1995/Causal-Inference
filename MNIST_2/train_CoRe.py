...
from models.BVAE import BVAE, ELBO, nll_fn, penalty_fn, CoRe_penalty_fn, mean_accuracy
import argparse
...
parser.add_argument('--CoRe_penalty_weight', type=float, default=1.0)
parser.add_argument('--epochs', type=int, default=10)
flags = parser.parse_args()


train_X = np.load('datasets/cmnist/fgbg_cmnist_cpr0.5-0.5/train_x.npy')
train_Y = np.load('datasets/cmnist/fgbg_cmnist_cpr0.5-0.5/train_y.npy')
test_X = np.load('datasets/cmnist/fgbg_cmnist_cpr0.5-0.5/test_x.npy')
test_Y = np.load('datasets/cmnist/fgbg_cmnist_cpr0.5-0.5/test_y.npy')
...
def run(batches_X, batches_Y, batches_counter_X, batches_counter_Y, train=True, epoch=0):
    ...
    for batch_X, batch_Y, \
        ...
        CoRe_penalty = CoRe_penalty_fn(logits.view(-1, 1), labels)
        ...
        loss = loss + flags.CoRe_penalty_weight*CoRe_penalty
