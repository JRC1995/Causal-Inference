...
def mini_batch(data_X, data_Y, data_X2, data_Y2, batch_size=64):
    ...
    while i < data_size:
        ...
        sampled_indices2 = random.sample(indices2, k=batch_size)

        batch_X = data_X[i:i+batch_size]
        batch_Y = data_Y[i:i+batch_size]

        batch_X2 = data_X2[sampled_indices2]
        batch_Y2 = data_Y2[sampled_indices2]

        batches_X.append(batch_X)
        batches_Y.append(batch_Y)

        batches_X2.append(batch_X2)
        batches_Y2.append(batch_Y2)

        i += batch_size

    return batches_X, batches_Y, batches_X2, batches_Y2
...
for restart in range(flags.n_restarts):
    ...
    def penalty_fn(logits_1, y_1, logits_2, y_2):
        ...
        if b1 == b2:
            for i in range(b1):
                grad_1 = autograd.grad(loss_1[i], [scale], create_graph=True)[0]
                grad_2 = autograd.grad(loss_2[i], [scale], create_graph=True)[0]
            unbiased_grad_norm = unbiased_grad_norm + torch.sum(torch.abs(grad_1*grad_2))
        else:
            grad_1 = autograd.grad(loss_1.mean(), [scale], create_graph=True)[0]
            grad_2 = autograd.grad(loss_2.mean(), [scale], create_graph=True)[0]
            unbiased_grad_norm = b1*torch.sum(torch.abs(grad_1*grad_2))

        return unbiased_grad_norm

    def run(batches_X, batches_Y, batches_X1, batches_X2, idx2env, train=True):
...
plt.xlabel('Epoch')
plt.savefig('IRM_mini_batch_accuracy_plot_9000.png')
plt.show()
