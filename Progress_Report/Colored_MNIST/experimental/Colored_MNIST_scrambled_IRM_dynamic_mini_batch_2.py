...
parser.add_argument('--lr', type=float, default=0.0001898536566546834)
parser.add_argument('--n_restarts', type=int, default=10)
parser.add_argument('--penalty_anneal_iters', type=int, default=200)
parser.add_argument('--penalty_weight', type=float, default=300)
...
def mini_batch_penalty_based_2(data_X, data_Y, batch_size=128):
    ...
    while i < data_size:
        ...
        if i+(batch_size//2) > data_size:
            batch_size = 2*(data_size-i)

        batch_X = data_X[i:i+batch_size//2]
        batch_Y = data_Y[i:i+batch_size//2]

        if i in early_indices_dict:
            choose_from_indices = early_indices
        elif i in middle_indices_dict:
            choose_from_indices = middle_indices
        else:
            choose_from_indices = end_indices

        chosen_indices = random.sample(choose_from_indices, k=batch_size//2)
        batch_X_ = [data_X[id] for id in chosen_indices]
        batch_Y_ = [data_Y[id] for id in chosen_indices]
        batch_X += batch_X_
        batch_Y += batch_Y_
...
def mini_batch_penalty_based(data_X, data_Y, batch_size=64):
...
for restart in range(flags.n_restarts):
    ...
    def penalty_fn_2(logits, y):

        logits_1 = logits[0:64]
        y_1 = y[0:64]
        ...
        unbiased_grad_norm = 0.0

        grad_1 = autograd.grad(loss_1.mean(), [scale], create_graph=True)[0]
        grad_2 = autograd.grad(loss_2.mean(), [scale], create_graph=True)[0]
        unbiased_grad_norm = torch.sum(torch.abs(grad_1*grad_2))

        return unbiased_grad_norm
    ...
    for step in range(flags.steps):
        ...
        if step == 9000:
            train_batches_X, train_batches_Y = mini_batch_penalty_based_2(train_X, train_Y)
        else:
